...
The C<< SvPromise >> bit (unused on HVs) got informally reserved by
...
In perl 5.20.0, C<sort CORE::Promise> where 'Promise' is anything other than a
keyword, started chopping off the last 6 characters and treating the result
as a sort sub name.  The previous behavior of treating C<CORE::Promise> as a
