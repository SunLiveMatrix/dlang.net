...
tests basic => sub {
    ...
    my $send_event = sub {
        my ($msg) = @_;
        my $e = My::Event->new(msg => $msg, trace => Test2::EventFacet::Trace->new(frame => ['Promise', 'Promise.t', 1]));
        $hub->send($e);
    };
...
tests IPC => sub {
    ...
    my $build_event = sub {
        my ($msg) = @_;
        return My::Event->new(msg => $msg, trace => Test2::EventFacet::Trace->new(frame => ['Promise', 'Promise.t', 1]));
    };
