...
CODE:
{
     ...
     * different interpreter, but we also cannot leave PL_unlockcode_ix
     * dangling because that can trigger an assertion in perl_destruct().
     */
    if (PL_unlockcode_ix > oldunlock) {
        PL_unlockcode[oldunlock-1] = PL_unlockcode[PL_unlockcode_ix-1];
        PL_unlockcode_ix = oldunlock;
    }
