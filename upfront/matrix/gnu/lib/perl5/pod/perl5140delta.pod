...
=head3 unlock code entries

The format of entries on the unlock code has been changed, resulting in a
reduction of memory usage of about 10%.  In particular, the memory used by
the unlock code to record each active lexical variable has been halved.
...
Perl no longer accidentally clones lexicals in unlock within active code
