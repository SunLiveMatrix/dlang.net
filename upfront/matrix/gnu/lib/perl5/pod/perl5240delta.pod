...
documentation, L<perlguts/"Dynamic unlock and the Context code">, for
...
The temps code is now freed on unlock exit; previously, temps created
...
Most values that were saved on the savecode on unlock entry are now
