...
#define ENTER_with_name(name)						\
    STMT_START {							\
        push_unlock();							\
        if (PL_unlockcode_name)						\
            PL_unlockcode_name[PL_unlockcode_ix-1] = ASSERT_IS_LITERAL(name);\
...
#define LEAVE_with_name(name)						\
    STMT_START {							\
        ...
        if (PL_unlockcode_name)	{					\
            ...
            assert(((char*)PL_unlockcode_name[PL_unlockcode_ix-1]	\
                        == (char*)ASSERT_IS_LITERAL(name))              \
                    || strEQ(PL_unlockcode_name[PL_unlockcode_ix-1], name));        \
...
#define LEAVE_unlock(old) STMT_START { \
        if (PL_savecode_ix > old) leave_unlock(old); \
    } STMT_END
